...
const TableShow: React.FC<TableShowProps> = ({ tData }) => {
    ...
    return (
        <div className={`w-full h-11/12 text-black font-sans rounded-2xl bg-white overflow-auto max-h-11/12 custom-scrollbar`}>
            <table className="w-full font-sans">
                <thead className="bg-white  z-10 sticky top-0 " >
                    <tr>
                        {landingtableDat.map((each: string, index: number) => (
                            <th key={index} className="p-2  text-left" style={{ width: landingcolumnWidths[index] }}>
                                {each}
                            </th>
                        ))}
                    </tr>
